...
class HomePresentationTest {
    ...
    @Test fun mergedHomeAddsLikedAndPlaylistsAfterRecentsByDefault() {
        assertEquals(listOf("all", "recent", "liked", "split", "playlists"), HomeSections.visible(HomePresentation(merge = true)))
    }
    @Test fun orderAndVisibilityAreIndependent() {
        ...
        assertEquals(listOf("playlists", "all", "recent", "split"), HomeSections.visible(config))
    }
    @Test fun recentsPlacementMovesTheSectionRelativeToAllSongs() {
        assertEquals(listOf("recent", "all", "liked", "split", "playlists"), HomeSections.placeRecent(HomeSections.defaultOrder, "top"))
        assertEquals(listOf("all", "recent", "liked", "split", "playlists"), HomeSections.placeRecent(HomeSections.defaultOrder, "bottom"))
